...
LINE_CONTENTS_PARSER = re.compile(
    r'^(<font color="##?[0-9a-f]{6,8}">)(.*)(</font>)(?: \[(.*)\])?', re.IGNORECASE
)
LINK_PARSER = re.compile(r'^<a href="([^"]+)">\(LINK\)</a>$')
LOG_TIMESTAMP = re.compile(
    r'(?:<a name="\d{4}-\d{1,2}-\d{1,2}" /><b>Session started at (\d{4})-(\d{1,2})-(\d{1,2}) / (\d\d):(\d\d)|<b>Chat log started at (\d{1,2})\.(\d{1,2})\.(\d{4}) / (\d\d):(\d\d):(\d\d))</b>'
)
    ...
    for l in lines:
        ...
        if timestamp_match:
            ...
            if timestamp_match.groups()[1]:
                ...
                _, _, _, _, _, month, day, year, hour, minute, second = (
                    timestamp_match.groups()
                )

            current_timestamp = "%s%s%s_%s%s%s" % (
                year,
                month,
                day,
                hour,
        ...
        if crunch and HIDDEN_ROLL_GM.match(contents):
            ...
            if display_crunch and crunch:
                line_group.append(
                    "%s%s: %s%s" % (font_start, contents, crunch, font_end)
                )
            else:
